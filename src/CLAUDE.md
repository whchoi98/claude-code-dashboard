...
│   ├── OrgProvider.tsx   # multi-org context — fetches /api/orgs ONCE (module-level promise; StrictMode-safe), exposes useOrg() → { org, setOrg, orgs, loading }; URL-synced ?org= (absent/invalid → 'primary'; while the list loads a raw ?org= deep link is accepted optimistically so the first fetches hit the right org). setOrg always deletes ?group= — group maps are per org. Wraps <Routes> in App.tsx OUTSIDE GroupScopeProvider. Layout renders the sidebar org switcher (segmented control above the language toggle) only when orgs.length > 1 — single-org deployments look byte-identical to before
