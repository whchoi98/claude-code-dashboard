...
Browser-side SPA. Renders the 20 dashboard pages, handles i18n / date range / email masking, and talks to the Express proxy under `/api`.
...
│   ├── Layout.tsx        # h-screen flex wrapper (sidebar pinned, main scrolls in its own pane); NAV array, language toggle, health badges, version badge → /changelog, static AWS run-rate label. MOBILE (< lg): the sidebar is a fixed slide-in drawer (navOpen state + backdrop, closes on nav tap) behind a sticky hamburger top bar; pages use responsive grids (grid-cols-2 lg:grid-cols-4 KPI rows, stacked chart pairs), p-4 lg:p-8 padding, and overflow-x-auto table wrappers. STANDALONE PWA safe-areas (pairs with index.html's viewport-fit=cover; env()=0 on desktop): the SPLIT is deliberate — main carries ONLY the bottom inset, side insets live on the sticky top bar (full-bleed background under the notch) + the Outlet wrapper (scrolled content), drawer pads all three edges; putting side insets on main double-counts the top bar's and clips its background short of the display edge. Fixed overlays (FloatingChat pill/panel, UserDetailPanel) carry their own env() offsets — main's padding can't reach position:fixed elements. NOTE viewport-fit=cover affects regular mobile Safari too, not just standalone
