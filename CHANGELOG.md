...
- **Compliance/audit feed pagination + warm cache**: `/api/compliance/activities` now uses the upstream API's actual cursor (`after_id` derived from each page's last event id), accepts `starting_date` for early termination, and returns `stop_reason` + `in_window` so the UI can warn when older events were truncated. A startup prewarm self-fetches the 7d / 14d / 30d windows on each ECS task boot and refreshes every 5 minutes — most users hit the upstream cache and see results in <1 s instead of paginating 30+ s of API calls. UI banner surfaces the cap when reached. See [ADR 0003 follow-ups](docs/decisions/0003-hybrid-live-cost.md).
...
- `.env` file permissions hardened from 664 to 600 (user-read-only). Project-policy already had `.env` in `.gitignore` and the Anthropic API keys had not been committed.
...
- **`.env` 파일 권한 강화 664 → 600** (사용자 read-only). 프로젝트 정책상 `.env`는 이미 `.gitignore` + Anthropic API 키가 git에 올라간 적 없음을 검증.
