...
| Method | Path | Notes |
|---|---|---|
| GET | `/api/groups` | Returns the group mapping: `{ source: "live"|"auto"|"empty", file, groups: string[], map: Record<emailLower, group> }`. `live` = latest admin CSV under `group-map/` (wins when present — carries admin-chosen names). **`auto`** = no CSV → the map is derived from live `user_cost_report` × `rbac_group_id` (default 31-day window; each user → their max-spend group, labeled with **real group names** via `GET /v1/compliance/groups` — `grp-<id suffix>` fallback — plus `group_ids: Record<label, rbac_group_id>` and `period`). Neither available → `{ source: "empty", groups: [], map: {} }` (200, not an error). Works without `ARCHIVE_S3_BUCKET` (auto path only needs the Analytics key). Raw lowercased emails for client-side matching; the UI still renders every email via `maskEmail`. Consumed by the `useGroupScope` hook + the sidebar Group-scope selector. |
