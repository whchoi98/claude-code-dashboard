...
| Method | Path | Notes |
|--------|------|-------|
| GET | `/api/analytics/summaries?starting_date=&ending_date=` | DAU/WAU/MAU, seat utilization, adoption rate. Server normalizes upstream `summaries` key → `data`. On a keyed deployment an upstream failure returns `{ source: "upstream_error", reason, data: [] }` — never mock rows (mock is keyless-dev only). |
| GET | `/api/analytics/users?date=` | Per-user engagement + Claude Code productivity for a single day. Keyed upstream failure → `source: "upstream_error"` + empty data (no mock). |
| GET | `/api/analytics/users/range?starting_date=&ending_date=` | **Whole-window serving (ADR-0019)**: S3-archive-first for EVERY requested day (columnar partitions + `inflateUser`; 24-wide pool), live-API fallback only for the newest ≤31 missing days inside the upstream's 90-day lookback (5-wide pool, one 429 retry); older misses come back as `source: "unarchived"` empty days, real S3 read failures as `source: "upstream_error"` with `error.type: "archive_error"`. Hard cap 366 days. Returns `days[]`, a `coverage` block (`requested_days` / `s3_days` / `live_days` / `unarchived_days` / `error_days` — the `RangeCoverageNote` banner renders it) and the legacy `cache` object (`s3_hits` / `live_calls`). |
| GET | `/api/analytics/skills?date=` | Distinct user counts per skill. Keyed upstream failure → `upstream_error` + empty data. |
| GET | `/api/analytics/skills/range?starting_date=&ending_date=` | Range fan-out of `/skills` with the same whole-window policy as `users/range`, served from the **raw sidecar** (`raw/skills/` — exact unflattened live-API rows incl. `invocation_count`, nested `*_metrics`, `attributed_list_price`; the columnar partitions drop those fields). Same `coverage` block. |
| GET | `/api/analytics/connectors?date=` | Distinct user counts per connector. Keyed upstream failure → `upstream_error` + empty data. |
| GET | `/api/analytics/connectors/range?starting_date=&ending_date=` | Same whole-window raw-sidecar policy as `skills/range`. |
| GET | `/api/analytics/projects?date=` | Chat project usage (`/apps/chat/projects`). Keyed upstream failure → `upstream_error` + empty data. |
| GET | `/api/analytics/projects/range?starting_date=&ending_date=` | Same whole-window raw-sidecar policy as `skills/range`. |
| GET | `/api/analytics/plugins/range?starting_date=&ending_date=` | Plugin install/invocation usage (`/analytics/plugins`), same whole-window raw-sidecar policy as `skills/range`. Raw sidecar starts at the v2.2 collector deploy (2026-08) — earlier days serve via the bounded live fallback or as `unarchived`. |
...
| `run_athena_sql` | S3 archive via Athena | One read-only `SELECT`/`WITH` over the six Glue tables (`claude_code_analytics`, `summaries_daily`, `skills_daily`, `connectors_daily`, `projects_daily`, `compliance_daily`). Goes through `sanitizeAthenaQuery`; results capped at 200 rows. Partition column `date` is `varchar` — use plain string literals, not `DATE '…'`. |
...
- **Date clamping**: the proxy clamps any incoming `starting_date` / `ending_date` to the engagement family's **dynamic finalization horizon** before calling the Analytics *engagement* + Admin upstreams (those APIs return HTTP 400 for newer dates, naming the newest served day). The horizon is learned per API key (`server/freshness.js`): an hourly `users?date=today−1` probe plus any 400 that names a day, with a static `today − 3` fallback until something is learned — typically it resolves to `today − 2` (upstream shortened the buffer in 2026-08; a day aggregates at 10:00 UTC the following day). `/api/health` reports the effective value as `dataConstraints.bufferDays`. Callers can pass `today` as `ending_date` on the picker side without triggering upstream errors; those responses simply cover up through the horizon. **Exception — the cost family**: `cost_report` / `usage_report` / `user_cost_report` serve the buffer days with *partial* data (verified live 2026-07-03), so the cost routes cover the full selected range up to `today`: `/api/cost/users` and `/api/cost/groups` resolve their windows via `resolveUserCostWindow` (ending ≤ today, inverted pairs pinned); `/api/cost/live` passes dates through `fetchCostSummary` (no clamp; defaults `[today−30, today]`). The upstream cost family rejects spans > 31 days — the server CHUNKS longer windows into ≤31-day segments and merges (ADR-0019; up to 186 days, `window_clamped: true` beyond), so the CSV path is only the fallback for live outages and pre-chunk-cap history. `/api/cost/efficiency` still clamps to `today - 3` deliberately, to stay window-aligned with its `users/range` productivity join. Compliance endpoints are NOT clamped — they're real-time.
