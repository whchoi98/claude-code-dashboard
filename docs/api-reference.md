...
`email→group` mapping for group-level dashboard scoping (v1.4.0; per-page GroupTabs since 2026-07). Groups come from an admin-uploaded CSV stored latest-wins at `s3://<archive>/group-map/` when present, otherwise they are **auto-derived** from the live `user_cost_report × rbac_group_id` dimension (shipped upstream 2026-07 — the original 400 "not yet supported" no longer applies; see ADR-0011). Cognito-edge-gated like the cost routes.

| Method | Path | Notes |
|---|---|---|
| GET | `/api/groups` | Returns the group mapping: `{ source: "live"|"auto"|"empty", file, groups: string[], map }`. `live` = latest admin CSV under `group-map/` (wins when present — carries admin-chosen names; map values are single-group **strings**). **`auto`** = no CSV → the map is derived from live `user_cost_report` × `rbac_group_id` (default 31-day window; map values are **arrays of every membership**, spend-desc — `[0]` = max-spend group; a single-value collapse used to erase groups that were nobody's top group. Labeled with **real group names** via `GET /v1/compliance/groups` — `grp-<id suffix>` fallback — plus `group_ids: Record<label, rbac_group_id>` and `period`). `groups` covers every membership. Neither available → `{ source: "empty", groups: [], map: {} }` (200, not an error). Works without `ARCHIVE_S3_BUCKET` (auto path only needs the Analytics key). Raw lowercased emails for client-side matching; the UI still renders every email via `maskEmail`. Consumed by the `useGroupScope` hook + the per-page `GroupTabs` selector (the client normalizes both map shapes to arrays). |
