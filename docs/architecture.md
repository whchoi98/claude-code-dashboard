...
| Server-side aggregation | `/api/cost/live` joins Analytics `cost_report` + `usage_report` on `(product, model)` and reshapes into the `CsvResp` shape consumed by the Cost page. `/api/cost/users` proxies `user_cost_report` (paginated, raw emails) to deliver live per-user USD spend. `/api/cost/efficiency` queries `user_cost_report` for the exact selected range and joins on `email` with `users/range` productivity — no activity-weighted scaling needed on the live path; falls back to the CSV path when live data is unavailable. `/api/cost/csv` keeps the manual Spend Report path for per-user token granularity + billing-grade reconciliation. See [ADR-0003](decisions/0003-hybrid-live-cost.md) and [ADR-0009](decisions/0009-live-user-cost.md). |
...
- **Live per-user cost via `user_cost_report`** — since v0.8.0, the Cost page's per-user "Top by Cost" table and `distinct_users` KPI are sourced live from `GET /api/cost/users` (Analytics `user_cost_report`). `/cost/efficiency` joins live per-user spend with `users/range` productivity on `email` for the exact selected range — no CSV-period activity-weighted scaling on this path. CSV upload is retained as optional for per-user token breakdowns + billing-grade reconciliation older than the live correction window. See [ADR-0003](decisions/0003-hybrid-live-cost.md) and [ADR-0009](decisions/0009-live-user-cost.md).
...
| 서버 사이드 집계 | `/api/cost/live`는 Analytics `cost_report` + `usage_report`를 `(product, model)` 단위로 조인해 Cost 페이지의 `CsvResp` 형태로 reshape. `/api/cost/users`는 `user_cost_report`를 페이지네이션하여 사용자별 라이브 USD spend를 제공(raw 이메일). `/api/cost/efficiency`는 선택 기간 그대로 `user_cost_report`를 조회해 `email` 기준으로 `users/range` 생산성 데이터와 조인 — 라이브 경로에서는 활동량 가중 분배 불필요; 라이브 데이터가 없으면 CSV 경로로 폴백. `/api/cost/csv`는 사용자별 토큰 상세 + 재무 정산을 위해 수동 Spend Report를 잔존. [ADR-0003](decisions/0003-hybrid-live-cost.md) 및 [ADR-0009](decisions/0009-live-user-cost.md) 참조. |
...
- **`user_cost_report` 기반 라이브 사용자별 비용** — v0.8.0부터 Cost 페이지의 per-user "Top by Cost" 테이블과 `distinct_users` KPI가 `GET /api/cost/users`(Analytics `user_cost_report`)에서 라이브로 공급됨. `/cost/efficiency`는 선택 기간 그대로 라이브 per-user spend를 `email` 기준으로 `users/range` 생산성 데이터와 조인 — 이 경로에서는 CSV 기간 활동량 가중 분배 불필요. CSV 업로드는 사용자별 토큰 상세 + 라이브 수정 윈도우보다 오래된 재무 정산을 위해 선택 사항으로 잔존. [ADR-0003](decisions/0003-hybrid-live-cost.md) 및 [ADR-0009](decisions/0009-live-user-cost.md) 참조.
