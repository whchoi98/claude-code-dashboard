...
- **>31-day windows: chunked cost reports + archive-first engagement ranges** — the upstream cost family caps spans at 31 days, so the server splits longer windows into ≤31-day chunks and merges exactly (daily buckets concat; per-user rows re-aggregate via `mergeUserReportRows`; 186-day cap with `window_clamped`), while the engagement `/range` routes serve their WHOLE window S3-archive-first (skills/connectors/projects from the raw sidecar — zero field loss) with the live API as a bounded newest-≤31-days fallback and a `coverage` block every page can render (`RangeCoverageNote`). Killed the silent last-31-days truncation, the Executive $0-on-error, and the keyed-mock-on-upstream-failure bug class; the Cost page loudly flags a CSV fallback whose export period doesn't cover the selection. See [ADR-0019](decisions/0019-over-31-day-windows.md).
- **Multi-org: a second subscription as a switchable source** — org ids `primary|org2` (`?org=`), per-request key resolution in `server/orgs.js`, org-prefixed caches/keep-warm (independent 60 rpm budgets), `org2/`-prefixed S3 + `*_org2` Glue tables, org-bound chatbot; single-org behavior byte-identical, infra gated by the committed `enableOrg2` cdk.json flag. See [ADR-0018](decisions/0018-multi-org-support.md).
...
Gaps tracked for future runbooks: rolling-deploy rollback, collector backfill, compliance prewarm cache flush, cost data reconciliation when live ≠ CSV, CloudFront invalidation after frontend deploys, audit-volume timeout diagnosis (partial:true rates / prewarm key drift — ADR-0016), gh-pages brochure publishing (`scripts/deploy-pages.sh`).
...
- **31일 초과 창: 비용 리포트 청크 분할 + 아카이브 우선 범위 서빙** — 업스트림 비용 계열은 요청당 31일 상한이므로 서버가 긴 창을 ≤31일 청크로 나눠 정확히 병합(일별 버킷 연결, 사용자별 행은 `mergeUserReportRows` 재집계, 186일 상한 + `window_clamped`), 엔게이지먼트 `/range` 계열은 전체 창을 S3 아카이브 우선으로 서빙(skills/connectors/projects는 raw 사이드카 — 필드 손실 없음) + 라이브 API는 최신 ≤31일 한정 폴백 + 페이지가 렌더하는 `coverage` 블록(`RangeCoverageNote`). 조용한 최근-31일 절단, Executive $0 렌더, keyed 배포의 mock 대체 버그 클래스를 제거했고, Cost 페이지는 CSV 폴백 기간이 선택 창을 커버하지 못하면 명시적으로 경고. [ADR-0019](decisions/0019-over-31-day-windows.md) 참조.
- **멀티 조직: 두 번째 구독을 전환 가능한 소스로** — org id `primary|org2`(`?org=`), `server/orgs.js`의 요청별 키 해석, org 접두 캐시/keep-warm(60rpm 예산 독립), `org2/` 프리픽스 S3 + `*_org2` Glue 테이블, org 바인딩 챗봇; 단일 조직 동작 바이트 동일, 인프라는 커밋된 `enableOrg2` cdk.json 플래그로 게이트. [ADR-0018](decisions/0018-multi-org-support.md) 참조.
...
향후 추가 후보: 롤링 배포 롤백, collector backfill, compliance prewarm 캐시 flush, 라이브 ≠ CSV 비용 데이터 정산 절차, 프런트 배포 후 CloudFront 무효화, 감사 볼륨 타임아웃 진단(partial:true 비율/프리웜 키 드리프트 — ADR-0016), gh-pages 브로셔 게시(`scripts/deploy-pages.sh`).
