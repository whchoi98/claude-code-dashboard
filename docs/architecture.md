...
- **Compliance members 엔드포인트 기반 그룹 멤버십** — 페이지별 그룹 스코프(GroupTabs)의 email→그룹 매핑은 `GET /v1/compliance/groups/{id}/members`(현재 시점의 확정 멤버십, 1h 캐시, all-or-nothing)에서 공급: 신규 그룹과 멤버 이동이 지출 누적을 기다리지 않고 1시간 안에 반영. `/api/groups` 소스 체인은 관리자 CSV(`live`) > 실제 멤버십(`members`) > 지출 파생 `user_cost_report × rbac_group_id`(`auto`) > last-good(`stale:true`) > `empty`. [ADR-0014](decisions/0014-membership-source-compliance-members.md) 참조.
