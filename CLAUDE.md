...
│   ├── pages/              19 routes (Overview, Executive, Users, UserProductivity, UserSearch, Trends, ClaudeCode, ClaudeChat, Cowork, Office, Design, Productivity, Agentic, Adoption, Cost, Compliance, Analyze, Archive, Changelog). Default date range is 7d. The DateRangeControl now allows today as the end date (engagement endpoints clamp to the 3-day buffer server-side; the cost family serves recent days with partial data). The '1d' preset anchors to the newest finalized day (today−3) except on Cost, which opts into `freshEnd` so '1d' = today.
