...
├── src/                    React SPA (Vite)
│   ├── components/         Shared UI (Layout, ClaudeIcon, KpiCard, ChartCard, PageHeader, LoadingState, UserDetailPanel, DateRangeControl, CsvUploader, Markdown, SortableTh, RangeCoverageNote). Layout pins the sidebar to the viewport via h-screen + per-pane overflow-y-auto so scrolling moves only the main pane; it also renders the version badge (links to /changelog) and the static AWS run-rate label.
│   ├── pages/              20 routes (Overview, Executive, Users, UserProductivity, UserSearch, Trends, ClaudeCode, ClaudeChat, Cowork, Office, Design, Productivity, Agentic, Adoption, Cost, CostLive, Compliance, Analyze, Archive, Changelog). CostLive = "비용 실시간": per-member MTD from the near-real-time Spend Limits API with totals, group scope, and a 60s auto-refresh that bypasses the server TTL via ?fresh=1. Default date range is 7d. The DateRangeControl now allows today as the end date (engagement endpoints clamp server-side to the DYNAMIC finalization horizon — typically today−2, learned hourly via server/freshness.js, today−3 fallback; the cost family serves recent days with partial data). The '1d' preset anchors to today−3 (the conservative fallback day — kept static because the compliance prewarm must predict the exact key the frontend sends) except on Cost, which opts into `freshEnd` so '1d' = today.
